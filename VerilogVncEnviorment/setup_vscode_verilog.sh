...
echo "Ensuring keyring dependencies are installed (dbus-x11, gnome-keyring, libsecret-tools)..."
apt-get update -y >/dev/null
apt-get install -y dbus-x11 gnome-keyring libsecret-tools >/dev/null
...
# Python snippet to enforce argv.json password-store setting for VS Code.
UPDATE_ARGV_PY=$(cat << 'EOF'
import json
import os
import re
import sys
...
for user_home in /home/*; do
    if [ -d "$user_home/.vnc" ]; then
        ...
        ARGV_FILE="$user_home/.config/Code/argv.json"
